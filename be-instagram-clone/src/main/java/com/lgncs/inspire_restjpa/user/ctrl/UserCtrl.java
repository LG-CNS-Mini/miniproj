...
import com.lgncs.inspire_restjpa.user.domain.dto.SseMessageResponseDTO;
...
public class UserCtrl {
    ...
    public ResponseEntity<UserResponseDTO> signin(UserRequestDTO request) {
        ...
        System.out.println(">>> user ctrl access  " + (String)(map.get("access")));
        System.out.println(">>> user ctrl refresh " + (String)(map.get("refresh")));
        System.out.println(">>> user ctrl unidirectional " + (UserResponseDTO)(map.get("response")));
        
        // sse push 일반 문자열 
        // sseCtrl.sendLoginPush(request.getEmail(), 
        //     "확인 할 메시지가 있습니다.") ; 
        
        // 객체
        sseCtrl.sendLoginPush(
            request.getEmail(),
            new SseMessageResponseDTO(
                "로그인 알림",
                "새로운 메시지가 도착했습니다.",
                "login",
                "/notifications/123" // 클릭 시 이동할 링크
            )
        );
